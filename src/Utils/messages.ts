...
export const generateWAMessageContent = async (
	...
	if (hasOptionalProperty(message, 'buttons') && !!(message as any).buttons) {
		...
		if ('text' in message) {
			...
			if ('caption' in message) buttonsMessage.contentText = (message as any).caption
		...
		const msg: any = { hydratedButtons: (message as any).templateButtons }
		if ('text' in message) {
			msg.hydratedContentText = (message as any).text
		} else {
			if ('caption' in message) msg.hydratedContentText = (message as any).caption
			Object.assign(msg, m)
		}
		if ('footer' in message && !!(message as any).footer) {
			msg.hydratedFooterText = (message as any).footer
		}
		m = {
			templateMessage: {
				fourRowTemplate: msg,
	...
	if (hasOptionalProperty(message, 'sections') && !!(message as any).sections) {
